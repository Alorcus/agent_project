...
def by_default_first(groups: list[Group]) -> list[Group]:
    """The default group first, then the rest by creation order — the
    ordering every `ConversationStore` implementation's `list_groups`
    promises."""
    return sorted(groups, key=lambda g: (g.kind != "default", g.created_at))
...
class InMemoryStore:
    ...
    async def save(self, conversation: Conversation) -> None:
        # No foreign key to do it for us — a stub that quietly permits what
        # the real store forbids would let I-1 break in every test using it.
